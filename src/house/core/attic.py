...
def clear_ridge_height(house: HouseSpec, roof: RoofSpec, attic: AtticSpec) -> float:
    ...
    A collar tie caps it like any other point: under the collar the clear height
    is the collar's, so the ceiling above it is headroom nobody can reach.
    Reporting the ceiling apex regardless would contradict `collar_blocks` and
    the drawing, which cuts the peak off flat.
