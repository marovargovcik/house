...
def test_bare_structure_reproduces_the_spec_worked_examples() -> None:
    ...
    """
    assert attic.usable_width(HOUSE, RoofSpec(pitch_deg=30.0), BARE) == pytest.approx(
        2.4182, abs=1e-4
    )
    assert attic.usable_width(HOUSE, RoofSpec(pitch_deg=40.0), BARE) == pytest.approx(
        4.4713, abs=1e-4
    )
...
def test_roof_buildup_costs_more_headroom_the_steeper_the_pitch() -> None:
    ...
    """
    assert attic.ceiling_drop(RoofSpec(pitch_deg=30.0), BUILT) == pytest.approx(
        0.30 / math.cos(math.radians(30.0))
    )
    assert attic.ceiling_drop(RoofSpec(pitch_deg=45.0), BUILT) == pytest.approx(
        0.4243, abs=1e-4
    )
...
def test_buildups_take_most_of_the_usable_strip() -> None:
    ...
    """
    assert attic.usable_width(HOUSE, RoofSpec(pitch_deg=30.0), BUILT) == pytest.approx(
        0.5254, abs=1e-4
    )
...
def test_a_shallow_pitch_leaves_no_standing_height_at_all() -> None:
    ...
    """
    result = attic.estimate(HOUSE, RoofSpec(pitch_deg=25.0), BUILT)
...
def test_knee_wall_buys_width() -> None:
    ...
    result = attic.estimate(HOUSE, RoofSpec(pitch_deg=30.0), kneed)
