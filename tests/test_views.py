...
def test_45_degrees_puts_the_apex_at_half_the_width() -> None:
    ...
    section = views.section(
        HOUSE, RoofSpec(pitch_deg=45.0, overhang_eave=0.0, overhang_gable=0.0), BARE
    )
...
def test_eave_overhang_descends_below_the_wall_top() -> None:
    ...
    pins against.
    """
    section = views.section(HOUSE, RoofSpec(pitch_deg=30.0), BARE)
...
def test_finished_surfaces_sit_inside_the_structure() -> None:
    ...
    """
    section = views.section(HOUSE, RoofSpec(pitch_deg=30.0), BUILT)
...
def test_a_collar_caps_the_standing_room_flat() -> None:
    ...
    section = views.section(HOUSE, RoofSpec(pitch_deg=45.0), kneed)
...
def test_plan_ridge_runs_out_over_the_rake_overhang() -> None:
    ...
    plan = views.plan(HOUSE, RoofSpec(pitch_deg=30.0), BARE)
